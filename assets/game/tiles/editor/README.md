...
Папки `terrain` и `water` содержат отдельные игровые тайлы 24×24. `source` хранит два утверждённых мастера 1536×1024 с сеткой 6×4 и исходной ячейкой 256×256. `transitions` содержит прозрачные прямые и внутренние диагональные края земли, песка и гравия. `preview` — увеличенный контрольный лист, исключённый из каталога конструктора.
...
В конструкторе весна, лето, осень и зима сохраняются как разные семейства травы. Летний `grass_lush` автоматически получает редкие цветочные варианты и детерминированные повороты, поэтому большая поляна не повторяет один очевидный квадрат. Кисть `water_clear` является кистью целого водоёма: по восьми соседям она сама выбирает внутреннюю воду, четыре берега, внешние и вогнутые углы, узкое русло, окончания и отдельный каменный пруд. Выбранное семейство хранится в JSON даже после подстановки другого PNG.

Грунтовые и каменные дороги принадлежат общему `RoadVisualSystem`: игра и конструктор используют одинаковые шестнадцать масок соседства `N/E/S/W`, повороты и два каталога по шесть отдельных PNG 24×24. На первой локации дорога уже входит в цельный мастер-арт, поэтому повторный непрозрачный слой модулей поверх него не рисуется; каталог применяется в редакторе и процедурных локациях.

Водные поверхности, берега, углы, узкое русло и пруд принадлежат `WaterVisualSystem`. Конструктор и процедурный renderer делят 19 отдельных модулей 24×24 и одну таблицу масок. Цельный мастер первой локации не перекрывается непрозрачными тайлами: живость добавляют только прозрачные CC0-эффекты пузырьков, рыбы и всплеска.
