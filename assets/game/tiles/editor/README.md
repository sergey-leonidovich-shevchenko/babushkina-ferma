...
В конструкторе `grass_lush` автоматически получает редкие цветочные варианты и детерминированные повороты, поэтому большая поляна не повторяет один очевидный квадрат. Кисть `water_clear` является кистью целого водоёма: по соседям она сама выбирает внутреннюю воду, четыре берега, внешние углы, узкое русло, окончания и отдельный каменный пруд. Выбранное семейство хранится в JSON даже после подстановки другого PNG.
