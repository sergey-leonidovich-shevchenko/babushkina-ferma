...
static func apply(game: Node, data: Dictionary) -> bool:
	...
	game.state.fishing.total_caught = int(fishing.get("caught", 0)); game.state.fishing.best_sizes = fishing.get("records", {}).duplicate(true)
