...
static func snapshot(game: Node) -> Dictionary:
	...
	for node in game.food_nodes:
		forage.append({"active":node.active,"ready_at":node.get("ready_at", 0.0),"stage":node.get("stage", -1),"kind":node.kind,"location":node.get("location", "overworld"),"position":[node.position.x,node.position.y],"planted_by_player":node.get("planted_by_player", false)})
	return {"version":VERSION,"state_schema":STATE_SCHEMA,"player":[game.player.x,game.player.y],"location":game.current_location,"day":game.day,"minutes":game.game_minutes,"energy":game.energy,"coins":game.coins,"xp":game.player_xp,"level":game.player_level,"hp":game.player_hp,"profile":{"character":game.state.player.profile.duplicate(true),"relationships":game.state.player.relationships.duplicate(true),"quest_memory":game.state.player.quest_memory.duplicate(true)},"inventory_meta":{"backpack_level":game.state.inventory.backpack_level,"durability":game.state.inventory.durability.duplicate(true)},"progression":{"points":game.skill_points,"levels":game.skill_levels.duplicate(true),"xp":game.skill_xp.duplicate(true),"talents":game.talent_levels.duplicate(true),"craft_count":game.state.player.craft_count,"mana":game.player_mana},"effects":{"strength":game.strength_timer,"regeneration":game.regeneration_timer,"speed":game.speed_timer,"invisibility":game.invisibility_timer,"defense":game.defense_timer},"fishing":{"caught":game.state.fishing.total_caught,"records":game.state.fishing.best_sizes.duplicate(true),"counts":game.state.fishing.catch_counts.duplicate(true),"qualities":game.state.fishing.best_qualities.duplicate(true),"perfect":game.state.fishing.perfect_catches,"best_control":game.state.fishing.best_control_streak,"traps":game.state.fishing.traps.duplicate(true)},"companions":{"recruited":game.recruited_companions.duplicate(),"active":game.active_companions.duplicate(),"command":game.state.player.companion_command,"bonds":game.state.player.companion_bonds.duplicate(true)},"storage":{"owned":game.home_chest_owned,"counts":game.home_chest_counts.duplicate(true)},"forge":game.forge_upgrades.duplicate(true),"contracts":{"day":game.state.contracts.offer_day,"statuses":game.state.contracts.statuses.duplicate(true),"completed":game.state.contracts.completed_total},"moon_glade":game.state.world.moon_glade.duplicate(true),"castle_campaign":game.state.world.castle_campaign.duplicate(true),"estate":game.state.world.estate.duplicate(true),"counts":game.export_inventory_counts().duplicate(true),"slots":game.inventory_slots.duplicate(true),"hotbar":game.hotbar_slots.duplicate(true),"equipment":game.equipment.duplicate(true),"quest_active":game.quest_active,"quest_complete":game.quest_complete,"missions":game.mission_states.duplicate(true),"tutorial":{"step":game.tutorial_step,"events":game.tutorial_events_completed.duplicate(true),"seen":game.seen_discoveries.duplicate(true),"animation":game.character_animation_directions.keys()},"weapons":{"sword":game.sword_crafted,"bow":game.has_bow,"crystal":game.has_crystal_sword,"equipped":game.equipped_weapon},"plots":plot_data,"world_plots":game.WorldFarmingSystem.snapshot(game),"resource_hits":game.resource_nodes.map(func(node): return node.hits),"trees":game.state.world.tree_nodes.map(func(tree): return {"health":tree.health,"stage":tree.stage,"regrow_timer":tree.regrow_timer}),"food_active":game.food_nodes.map(func(node): return node.active),"forage":forage,"enemies":game.enemy_nodes.map(func(enemy): return {"hp":enemy.hp,"alive":enemy.alive,"level":enemy.level,"position":[enemy.position.x,enemy.position.y],"direction":[enemy.direction.x,enemy.direction.y],"attack_timer":enemy.attack_timer}),"hazards":game.hazard_nodes.map(func(hazard): return {"cooldown":hazard.cooldown}),"wildlife":wildlife,"world_loot_seed":game.world_loot_seed,"containers":containers,"drops":drops}
...
static func apply(game: Node, data: Dictionary) -> bool:
	...
	game.state.player.profile = profile.get("character", game.state.player.profile).duplicate(true); game.state.player.relationships = profile.get("relationships", {}).duplicate(true); game.state.player.quest_memory = profile.get("quest_memory", {}).duplicate(true)
	var inventory_meta: Dictionary = data.get("inventory_meta", {})
	game.state.inventory.backpack_level = int(inventory_meta.get("backpack_level", 0)); game.state.inventory.durability.merge(inventory_meta.get("durability", {}), true); game.state.inventory.normalize_meta()
	...
	game.talent_levels = game.TalentSystem.merge_levels(progression.get("talents", {}))
	game.state.player.craft_count = maxi(int(progression.get("craft_count", 0)), 0)
	game.player_mana = progression.get("mana", game.player_mana)
	var effects: Dictionary = data.get("effects", {})
	game.strength_timer = effects.get("strength", 0.0); game.regeneration_timer = effects.get("regeneration", 0.0); game.speed_timer = effects.get("speed", 0.0)
	game.invisibility_timer = effects.get("invisibility", 0.0); game.defense_timer = effects.get("defense", 0.0)
	var fishing: Dictionary = data.get("fishing", {})
	game.state.fishing.total_caught = int(fishing.get("caught", 0)); game.state.fishing.best_sizes = fishing.get("records", {}).duplicate(true); game.state.fishing.catch_counts = fishing.get("counts", {}).duplicate(true); game.state.fishing.best_qualities = fishing.get("qualities", {}).duplicate(true); game.state.fishing.perfect_catches = int(fishing.get("perfect", 0)); game.state.fishing.best_control_streak = float(fishing.get("best_control", 0.0)); game.state.fishing.traps = fishing.get("traps", []).duplicate(true)
	...
	game.state.world.moon_glade = game.MoonGladeSystem.normalize_state(data.get("moon_glade", {}))
	game.state.world.castle_campaign = game.CastleCampaignSystem.normalize_state(data.get("castle_campaign", {}))
	game.state.world.estate = game.EstateSystem.normalize_state(data.get("estate", {}))
	...
	game.state.player.companion_command = String(companions.get("command", "follow")); game.state.player.companion_bonds = companions.get("bonds", {}).duplicate(true)
	game.state.player.normalize()
	...
	game.WorldFarmingSystem.restore(game, data.get("world_plots", []))
	...
	for index in mini(data.get("trees", []).size(), game.state.world.tree_nodes.size()):
		game.state.world.tree_nodes[index].health = data.trees[index].health; game.state.world.tree_nodes[index].stage = data.trees[index].stage; game.state.world.tree_nodes[index].regrow_timer = data.trees[index].regrow_timer
	if data.has("forage"):
		while game.food_nodes.size() < data.forage.size():
			var saved_dynamic: Dictionary = data.forage[game.food_nodes.size()]
			var saved_kind := String(saved_dynamic.get("kind", "apple"))
			if not game.ForageSystem.TYPES.has(saved_kind): saved_kind = "apple"
			var saved_position: Array = saved_dynamic.get("position", [400.0, 400.0])
			game.food_nodes.append({"active":false,"ready_at":0.0,"stage":0,"kind":saved_kind,"location":String(saved_dynamic.get("location", "overworld")),"position":Vector2(saved_position[0],saved_position[1]),"planted_by_player":true})
		for index in mini(data.forage.size(), game.food_nodes.size()):
			...
			if int(data.forage[index].get("stage", -1)) >= 0: game.food_nodes[index].stage = clampi(int(data.forage[index].stage),0,3)
	else:
