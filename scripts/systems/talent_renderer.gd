...
static func draw(game: Node2D) -> void:
	...
	game.draw_ui_string(game.UI_FONT, TITLE_RIBBON.position + Vector2(24, 39), game.TalentSystem.word(game, "title"), HORIZONTAL_ALIGNMENT_CENTER, TITLE_RIBBON.size.x - 48, 20, UiKitSystem.COLORS.text_light)
...
static func draw_tree_header(game: Node2D) -> void:
	for group_index in game.TalentSystem.GROUPS.size():
		...
		game.draw_ui_string(game.UI_FONT, header.position + Vector2(10, 27), "%s  %s" % [group.icon, game.TalentSystem.word(game, "group_%s" % group.id)], HORIZONTAL_ALIGNMENT_CENTER, header.size.x - 20, 10, Color("fff0cf"))
	UiKitSystem.draw_nine_patch(game, "badge", Rect2(874, 62, 168, 42))
	game.draw_ui_string(game.UI_FONT, Vector2(888, 89), game.LocaleSystem.ui("level_points", [game.player_level, game.skill_points]).to_upper(), HORIZONTAL_ALIGNMENT_CENTER, 140, 9, Color("70452b"))
...
static func draw_node(game: Node2D, index: int) -> void:
	...
	game.draw_ui_string(game.UI_FONT, rect.position + Vector2(11, 30), String(talent.icon), HORIZONTAL_ALIGNMENT_CENTER, 26, 13, Color("70452b"))
	game.draw_ui_string(game.UI_FONT, rect.position + Vector2(46, 20), game.TalentSystem.word(game, talent_id), HORIZONTAL_ALIGNMENT_LEFT, 128, 10, UiKitSystem.COLORS.ink)
	...
	game.draw_ui_string(game.UI_FONT, rect.position + Vector2(46, 36), status, HORIZONTAL_ALIGNMENT_LEFT, 128, 8, Color("526b42") if learned else Color("80563b"))
...
static func draw_selected_details(game: Node2D) -> void:
	...
	game.draw_ui_string(game.UI_FONT, rect.position + Vector2(20, 28), game.TalentSystem.word(game, String(talent.id)), HORIZONTAL_ALIGNMENT_LEFT, 488, 15, UiKitSystem.COLORS.ink)
	...
	game.draw_ui_string(game.UI_FONT, rect.position + Vector2(20, 118), hint, HORIZONTAL_ALIGNMENT_LEFT, 480, 9, Color("80593a"))
	...
	game.draw_ui_string(game.UI_FONT, RESPEC_BUTTON.position + Vector2(10, 25), game.TalentSystem.word(game, "respec", false, [game.TalentSystem.RESPEC_COST]), HORIZONTAL_ALIGNMENT_CENTER, RESPEC_BUTTON.size.x - 20, 8, UiKitSystem.COLORS.text_light)
	game.draw_ui_string(game.UI_FONT, Vector2(856, 514), game.TalentSystem.word(game, "close"), HORIZONTAL_ALIGNMENT_RIGHT, 216, 8, Color("76543b"))
