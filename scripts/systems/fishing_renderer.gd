...
static func _draw_cast_meter(game: Node2D, state: RefCounted) -> void:
	...
	game.draw_ui_string(game.UI_FONT, Vector2(792, 183), game.LocaleSystem.text("fish_cast_title"), HORIZONTAL_ALIGNMENT_CENTER, 282, 20, Color("fff0bd"))
	...
	game.draw_ui_string(game.UI_FONT, Vector2(794, 258), game.LocaleSystem.text("fish_release"), HORIZONTAL_ALIGNMENT_CENTER, 278, 14, Color("cbe9df"))
...
static func _draw_minigame(game: Node2D, state: RefCounted) -> void:
	...
	game.draw_ui_string(game.UI_FONT, Vector2(792, 139), game.LocaleSystem.text("fish_minigame_title"), HORIZONTAL_ALIGNMENT_CENTER, 282, 20, Color("fff0bd"))
	...
	game.draw_ui_string(game.UI_FONT, Vector2(995, 238), control_lines[0], HORIZONTAL_ALIGNMENT_CENTER, 79, 13, Color("d6eadb"))
	game.draw_ui_string(game.UI_FONT, Vector2(995, 258), control_lines[1], HORIZONTAL_ALIGNMENT_CENTER, 79, 13, Color("d6eadb"))
	game.draw_ui_string(game.UI_FONT, Vector2(995, 292), "%d%%" % roundi(state.catch_progress * 100.0), HORIZONTAL_ALIGNMENT_CENTER, 79, 18, Color("fff0bd"))
	if state.perfect: game.draw_ui_string(game.UI_FONT, Vector2(995, 337), "★", HORIZONTAL_ALIGNMENT_CENTER, 79, 28, Color("f6d85d"))
...
static func _draw_result(game: Node2D, state: RefCounted) -> void:
	...
	game.draw_ui_string(game.UI_FONT, Vector2(738, 232), game.LocaleSystem.text("fish_catch_title") if state.catch_progress >= 1.0 else game.LocaleSystem.text("fish_escaped"), HORIZONTAL_ALIGNMENT_CENTER, 336, 20, Color("fff0bd"))
	game.draw_ui_string(game.UI_FONT, Vector2(738, 270), state.result_text, HORIZONTAL_ALIGNMENT_CENTER, 336, 15, Color("d6eadb"))
