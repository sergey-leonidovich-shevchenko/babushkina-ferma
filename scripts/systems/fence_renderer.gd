...
static func draw_ui(game: Node2D) -> void:
	...
	if not game.FenceSystem.active(game):
		if game.touch_controls_visible and game.current_location in game.FenceSystem.BUILDABLE_LOCATIONS:
			var launcher: Rect2=game.FenceSystem.TOUCH_BUTTON; game.draw_rect(launcher,Color(0.18,0.11,0.055,0.92)); game.draw_rect(launcher,Color("d7a94f"),false,2.0); game.draw_item_icon(game.FenceSystem.SECTION_ITEM,launcher.grow(-8)); game.draw_string(game.UI_FONT,launcher.position+Vector2(5,52),"Z",HORIZONTAL_ALIGNMENT_LEFT,18,11,Color("fff0c8"))
		return
	...
	game.draw_string(game.MENU_FONT,rect.position+Vector2(16,28),game.LocaleSystem.ui("fence_builder"),HORIZONTAL_ALIGNMENT_LEFT,360,18,Color("fff0c8"))
	...
	game.draw_string(game.UI_FONT,rect.position+Vector2(16,55),"%s · %s"%[game.FenceSystem.style_name(game,value.style),piece],HORIZONTAL_ALIGNMENT_LEFT,360,15,Color("ffe09a")); game.draw_string(game.UI_FONT,rect.position+Vector2(16,78),counts,HORIZONTAL_ALIGNMENT_LEFT,360,12,Color("d9c59c")); game.draw_string(game.UI_FONT,rect.position+Vector2(16,101),game.LocaleSystem.ui("fence_controls"),HORIZONTAL_ALIGNMENT_LEFT,360,11,Color("eee0c2"))
