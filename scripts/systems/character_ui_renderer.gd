...
static func draw(game: Node2D) -> void:
	...
	game.draw_string(game.UI_FONT, Vector2(72, 129), word(game, "character"), HORIZONTAL_ALIGNMENT_LEFT, 210, 13, Color("fff0cf"))
	...
	game.draw_string(game.UI_FONT, Vector2(68, 262), String(profile.get("name", "Герой")), HORIZONTAL_ALIGNMENT_CENTER, 224, 17, UiKitSystem.COLORS.text_light)
	game.draw_string(game.UI_FONT, Vector2(68, 282), game.AdventurePolishRenderer.specialization_name(game, String(profile.get("specialization", "farmer"))), HORIZONTAL_ALIGNMENT_CENTER, 224, 10, Color("e4c686"))
	draw_xp(game)
	draw_resources(game)
	draw_stats(game)
	draw_effects(game)
...
static func draw_xp(game: Node2D) -> void:
	...
	game.draw_string(game.UI_FONT, Vector2(80, 307), label, HORIZONTAL_ALIGNMENT_CENTER, 200, 9, Color("fff0cf"))
...
static func draw_resources(game: Node2D) -> void:
	...
	for index in values.size():
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(13, 19), "%s  %d/%d" % [row[0], row[1], row[2]], HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 26, 9, Color("fff0cf"))
...
static func draw_stats(game: Node2D) -> void:
	...
	for index in stats.size():
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(4, 18), "%s %s" % [word(game, stats[index][0]), stats[index][1]], HORIZONTAL_ALIGNMENT_CENTER, 62, 8, Color("68432a"))
...
static func draw_effects(game: Node2D) -> void:
	game.draw_string(game.UI_FONT, Vector2(70, 456), word(game, "effects"), HORIZONTAL_ALIGNMENT_LEFT, 100, 9, Color("e9cf96"))
	var effects := effect_entries(game)
	if effects.is_empty():
		game.draw_string(game.UI_FONT, Vector2(158, 456), word(game, "none"), HORIZONTAL_ALIGNMENT_RIGHT, 132, 8, Color("a99778"))
		return
	for index in effects.size():
		var rect := Rect2(174 + index * 23, 442, 22, 22)
		UiKitSystem.draw_nine_patch(game, "badge", rect)
		game.draw_string(game.UI_FONT, rect.position + Vector2(1, 15), String(effects[index][0]), HORIZONTAL_ALIGNMENT_CENTER, 20, 8, Color("633c28"))
		game.draw_string(game.UI_FONT, rect.position + Vector2(-1, 28), "%.0f" % float(effects[index][1]), HORIZONTAL_ALIGNMENT_CENTER, 24, 7, Color("fff0cf"))
...
static func draw_companions(game: Node2D) -> void:
	game.draw_string(game.UI_FONT, Vector2(70, 482), "%s  %d/%d" % [word(game, "companions"), game.active_companions.size(), game.CompanionSystem.capacity(game)], HORIZONTAL_ALIGNMENT_LEFT, 220, 9, Color("e9cf96"))
	...
	game.draw_string(game.UI_FONT, COMMAND_BUTTON.position + Vector2(10, 23), "L1 / C  •  %s" % command, HORIZONTAL_ALIGNMENT_CENTER, COMMAND_BUTTON.size.x - 20, 9, UiKitSystem.COLORS.text_light)
