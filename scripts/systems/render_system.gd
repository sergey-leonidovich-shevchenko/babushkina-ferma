...
static func draw(game: Node2D) -> void:
	...
	game.draw_enemy_nodes_and_gate(); game.draw_resource_nodes(); game.draw_world_loot(); game.draw_wildlife(); game.draw_dropped_items(); game.draw_player(); game.draw_interaction_highlight()
