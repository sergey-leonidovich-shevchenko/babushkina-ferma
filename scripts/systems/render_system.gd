...
static func draw(game: Node2D) -> void:
	...
	if game.BuildingSystem.is_interior(game.current_location):
		...
		game.draw_farm()
	game.LevelEditorRenderer.draw_layer(game,"background"); game.LevelEditorRenderer.draw_layer(game,"ground")
	...
	game.CastleCampaignRenderer.draw(game); game.draw_hazards(); game.draw_enemy_nodes_and_gate(); game.draw_tree_nodes(); game.draw_resource_nodes(); game.draw_food_nodes(); game.draw_world_loot(); game.draw_wildlife(); game.draw_dropped_items(); FishingRenderer.draw_traps(game); game.draw_companions(); game.FarmLifeRenderer.draw_world(game); game.FirstChapterRenderer.draw_world(game); game.FenceRenderer.draw_world(game); game.LevelEditorRenderer.draw_layer(game,"objects"); game.draw_player(); game.VillageForegroundRenderer.draw(game); game.AdventurePolishRenderer.draw_world(game); game.draw_interaction_highlight(); game.LevelEditorRenderer.draw_layer(game,"foreground"); game.DebugPlaygroundRenderer.draw_world_overlay(game); game.DebugOverlayRenderer.draw_world(game)
	...
	if not clean_level_capture and not bool(game.FarmLifeSystem.state(game).photo_mode) and not game.LevelEditorSystem.active(game): game.draw_ui(); FishingRenderer.draw(game); game.WorldMapRenderer.draw(game); game.AdventurePolishRenderer.draw_ui(game); game.FenceRenderer.draw_ui(game)
	if not clean_level_capture and not game.LevelEditorSystem.active(game): game.FarmLifeRenderer.draw_ui(game); game.FirstChapterRenderer.draw_ui(game); game.DebugPlaygroundRenderer.draw_overlay(game); game.DebugOverlayRenderer.draw_panel(game)
