...
static func draw(game: Node2D) -> void:
	...
	game.CastleCampaignRenderer.draw(game); game.draw_hazards(); game.draw_enemy_nodes_and_gate(); game.draw_tree_nodes(); game.draw_resource_nodes(); game.draw_food_nodes(); game.draw_world_loot(); game.draw_wildlife(); game.draw_dropped_items(); game.draw_companions(); game.draw_player(); game.VillageForegroundRenderer.draw(game); game.AdventurePolishRenderer.draw_world(game); game.draw_interaction_highlight(); game.DebugPlaygroundRenderer.draw_world_overlay(game)
	game.draw_set_transform(Vector2.ZERO); game.AtmosphereRenderer.draw(game); game.draw_ui(); FishingRenderer.draw(game); game.WorldMapRenderer.draw(game); game.AdventurePolishRenderer.draw_ui(game); game.DebugPlaygroundRenderer.draw_overlay(game)
