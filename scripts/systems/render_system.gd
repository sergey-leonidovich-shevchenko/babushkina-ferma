...
static func draw(game: Node2D) -> void:
	...
	game.draw_hazards(); game.draw_enemy_nodes_and_gate(); game.draw_tree_nodes(); game.draw_resource_nodes(); game.draw_food_nodes(); game.draw_world_loot(); game.draw_wildlife(); game.draw_dropped_items(); game.draw_companions(); game.draw_player(); game.draw_interaction_highlight()
	game.draw_set_transform(Vector2.ZERO); game.AtmosphereRenderer.draw(game); game.draw_ui(); FishingRenderer.draw(game)
