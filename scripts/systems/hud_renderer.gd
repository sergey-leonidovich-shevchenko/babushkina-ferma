...
static func draw_menu_button(game: Node, ui, texture: Texture2D, rect: Rect2, label: String, badge: int) -> void:
	...
	if hovered:
		var clean := label.get_slice(" • ", 1) if " • " in label else label; game.draw_rect(Rect2(rect.position.x - 4, 98, rect.size.x + 8, 22), Color(0.18, 0.10, 0.05, 0.92)); game.draw_string(game.UI_FONT, Vector2(rect.position.x, 113), clean, HORIZONTAL_ALIGNMENT_CENTER, rect.size.x, 9, Color("ffe4a2"))
...
static func draw_action_button(game: Node, ui, rect: Rect2, label: String, enabled: bool, pressed: bool, dodge: bool) -> void:
	...
	ui.draw_atlas_piece(game, ui.CONTROL_ATLAS, rect.grow(8), sources[state_index]); game.draw_string(game.UI_FONT, rect.position + Vector2(2, 47), label, HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 4, 8, Color("ffe7ad") if enabled else Color("8f918b"))
...
static func draw_interaction_prompt(game: Node, ui) -> void:
	...
	ui.draw_atlas_piece(game, ui.CONTROL_ATLAS, ui.INTERACTION_PROMPT, Rect2(940, 96, 300, 184)); game.draw_string(game.UI_FONT, ui.INTERACTION_PROMPT.position + Vector2(48, 37), game.LocaleSystem.ui("action"), HORIZONTAL_ALIGNMENT_CENTER, 228, 12, Color("4b3424"))
