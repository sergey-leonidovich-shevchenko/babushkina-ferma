...
static func draw(game: Node2D) -> void:
	...
	game.draw_string(game.UI_FONT, TITLE_RIBBON.position + Vector2(24, 39), game.LocaleSystem.ui("world_guide"), HORIZONTAL_ALIGNMENT_CENTER, TITLE_RIBBON.size.x - 48, 19, UiKitSystem.COLORS.text_light)
...
static func draw_tabs(game: Node2D) -> void:
	for index in TAB_RECTS.size():
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(10, 28), "%s  %s" % [PAGE_ICONS[index], game.LocaleSystem.ui(PAGE_LABEL_KEYS[index])], HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 20, 10, Color("fff0cf"))
...
static func draw_map(game: Node2D) -> void:
	...
	for location in game.WorldMapSystem.LOCATIONS:
		...
		game.draw_string(game.UI_FONT, position + Vector2(-56, 28), label, HORIZONTAL_ALIGNMENT_CENTER, 112, 8, Color("f8f1dc"))
	UiKitSystem.draw_nine_patch(game, "tooltip", Rect2(838, 174, 232, 392))
	game.draw_string(game.UI_FONT, Vector2(858, 207), game.LocaleSystem.location(current), HORIZONTAL_ALIGNMENT_LEFT, 192, 16, UiKitSystem.COLORS.ink)
	...
	game.draw_string(game.UI_FONT, Vector2(920, 251), game.LocaleSystem.ui("season_" + game.WorldEventSystem.season(game.day)), HORIZONTAL_ALIGNMENT_LEFT, 130, 12, Color("67452e"))
	game.draw_string(game.UI_FONT, Vector2(858, 306), game.LocaleSystem.ui("map_legend"), HORIZONTAL_ALIGNMENT_LEFT, 192, 9, Color("765239"))
...
static func draw_calendar(game: Node2D) -> void:
	UiKitSystem.draw_panel(game, CONTENT, false)
	game.draw_string(game.UI_FONT, Vector2(88, 190), game.LocaleSystem.ui("guide_week_forecast"), HORIZONTAL_ALIGNMENT_LEFT, 420, 15, Color("fff0cf"))
	for offset in 7:
		var day: int = game.day + offset
		var rect := Rect2(82 + offset * 141, 204, 131, 174)
		UiKitSystem.draw_nine_patch(game, "tooltip", rect)
		game.draw_string(game.UI_FONT, rect.position + Vector2(10, 27), game.LocaleSystem.ui("day_short", [day]), HORIZONTAL_ALIGNMENT_CENTER, 111, 11, UiKitSystem.COLORS.ink)
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(10, 110), game.LocaleSystem.ui("weather_" + weather), HORIZONTAL_ALIGNMENT_CENTER, 111, 10, UiKitSystem.COLORS.ink)
		game.draw_string(game.UI_FONT, rect.position + Vector2(10, 131), game.LocaleSystem.ui("season_" + game.WorldEventSystem.season(day)), HORIZONTAL_ALIGNMENT_CENTER, 111, 9, Color("735239"))
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(20, 159), event_name, HORIZONTAL_ALIGNMENT_CENTER, 91, 7, Color("68432a"))
	game.draw_string(game.UI_FONT, Vector2(88, 408), game.LocaleSystem.ui("guide_valley_year"), HORIZONTAL_ALIGNMENT_LEFT, 360, 12, Color("fff0cf"))
	for index in 28:
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(5, 20), "%d" % (index + 1), HORIZONTAL_ALIGNMENT_CENTER, 52, 9, Color("55331f"))
		var season: String = game.WorldEventSystem.season(index + 1)
		game.draw_string(game.UI_FONT, rect.position + Vector2(5, 38), game.LocaleSystem.ui("season_" + season).left(3).to_upper(), HORIZONTAL_ALIGNMENT_CENTER, 52, 7, Color("795538"))
...
static func draw_bestiary(game: Node2D) -> void:
	...
	for index in kinds.size():
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(88, 28), String(data.name) if known else game.LocaleSystem.ui("guide_unknown_enemy"), HORIZONTAL_ALIGNMENT_LEFT, 210, 11, UiKitSystem.COLORS.ink if known else Color("725f4d"))
		game.draw_string(game.UI_FONT, rect.position + Vector2(88, 49), "УР. 1–5  •  HP %d  •  УРОН %d" % [data.hp, data.damage], HORIZONTAL_ALIGNMENT_LEFT, 210, 8, Color("765239") if known else Color("8f7c66"))
		if known:
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(88, 83), "+%d XP" % int(data.xp) if known else game.LocaleSystem.ui("guide_find_enemy"), HORIZONTAL_ALIGNMENT_LEFT, 118, 8, Color("9a672e") if known else Color("725f4d"))
...
static func draw_collections(game: Node2D) -> void:
	...
	game.draw_string(game.UI_FONT, Vector2(86, 190), "%s  •  %d/%d" % [game.LocaleSystem.ui("guide_fish_collection"), caught, game.FishingSystem.FISH_CATALOG.size()], HORIZONTAL_ALIGNMENT_LEFT, 600, 14, Color("fff0cf"))
	for index in game.FishingSystem.FISH_CATALOG.size():
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(84, 33), game.LocaleSystem.text(String(fish.name_key)) if record > 0 else "???", HORIZONTAL_ALIGNMENT_LEFT, 136, 11, Color("fff0cf"))
		game.draw_string(game.UI_FONT, rect.position + Vector2(84, 57), game.LocaleSystem.ui("guide_record", [record]) if record > 0 else game.LocaleSystem.ui("guide_not_caught"), HORIZONTAL_ALIGNMENT_LEFT, 136, 8, Color("d0a85f") if record > 0 else Color("9b8a70"))
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(84, 79), condition if not condition.is_empty() else game.LocaleSystem.ui("guide_all_year"), HORIZONTAL_ALIGNMENT_LEFT, 136, 7, Color("bca57c"))
...
static func draw_recipes(game: Node2D) -> void:
	...
	for visible_index in 9:
		...
		game.draw_string(game.UI_FONT, rect.position + Vector2(54, 28), game.inventory_item_name(String(recipe.output)), HORIZONTAL_ALIGNMENT_LEFT, 310, 11, UiKitSystem.COLORS.ink)
	...
	game.draw_string(game.UI_FONT, Vector2(582, 403), game.inventory_item_name(String(recipe.output)), HORIZONTAL_ALIGNMENT_CENTER, 454, 18, UiKitSystem.COLORS.ink)
	...
	game.draw_string(game.UI_FONT, Vector2(588, 526), game.LocaleSystem.ui("guide_recipe_open") if unlocked else game.LocaleSystem.ui("guide_recipe_locked"), HORIZONTAL_ALIGNMENT_CENTER, 442, 11, Color("66804c") if unlocked else Color("a83f2a"))
