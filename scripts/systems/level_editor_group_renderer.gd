...
static func draw_panel(game: Node2D, state: Dictionary) -> void:
	...
	for index in GroupSystem.LAYERS.size():
		var layer:String=GroupSystem.LAYERS[index]; var rect:=Rect2(GroupSystem.ROWS_START+Vector2(0,index*GroupSystem.ROW_HEIGHT),Vector2(200,32)); game.DebugUiKitSystem.draw_catalog_row(game,rect,String(state.layer)==layer)
		game.draw_ui_string(game.UI_FONT,rect.position+Vector2(8,21),{"foreground":"ПЕРЕДНИЙ","objects":"ОБЪЕКТЫ","ground":"ЗЕМЛЯ","background":"ФОН"}[layer],HORIZONTAL_ALIGNMENT_LEFT,100,11,TEXT)
		game.draw_ui_string(game.UI_FONT,rect.position+Vector2(108,21),"◉" if bool(state.layer_visibility.get(layer,true)) else "○",HORIZONTAL_ALIGNMENT_CENTER,40,14,SELECTED); game.draw_ui_string(game.UI_FONT,rect.position+Vector2(156,21),"◆" if bool(state.layer_locked.get(layer,false)) else "◇",HORIZONTAL_ALIGNMENT_CENTER,40,12,SELECTED)
