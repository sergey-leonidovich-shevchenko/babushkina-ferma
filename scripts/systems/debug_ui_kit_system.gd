...
static func draw_button(game: Node2D, rect: Rect2, label: String, active: bool, enabled: bool = true, editor: bool = false) -> void:
	...
	var fitted:=fit_label(game.UI_FONT,label,rect.size.x-24.0,11,8); var color:=EDITOR_BUTTON_TEXT if editor and enabled else (EDITOR_BUTTON_DISABLED if editor else (Color("fff0c8") if enabled else Color("88928d")))
	game.draw_ui_string(game.UI_FONT,rect.position+Vector2(12.0,rect.size.y*0.65),String(fitted.text),HORIZONTAL_ALIGNMENT_CENTER,rect.size.x-24.0,int(fitted.size),color)
