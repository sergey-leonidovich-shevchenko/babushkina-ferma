...
static func draw_button(game: Node2D, rect: Rect2, label: String, active: bool, enabled: bool = true, editor: bool = false) -> void:
	...
	game.draw_ui_string(game.UI_FONT,rect.position+Vector2(19.0,rect.size.y*0.62),label,HORIZONTAL_ALIGNMENT_CENTER,rect.size.x-27.0,10,Color("fff0c8") if enabled else Color("88928d"))
...
static func draw_catalog_row(game: Node2D, rect: Rect2, selected: bool) -> void:
	UiKitSystem.draw_button(game,rect,selected,true,game.settings_state.reduced_motion,Time.get_ticks_msec())
	game.draw_rect(rect.grow(-4.0),Color(TECH_GOLD,0.16 if selected else 0.05))
