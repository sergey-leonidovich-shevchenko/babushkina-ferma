...
func _initialize() -> void:
	...
	FarmLifeSuite.new(self).run(); FirstChapterSuite.new(self).run(); WorldFarmingSuite.new(self).run(); LevelEditorSuite.new(self).run(); FenceBuildingSuite.new(self).run()
